...
class UsersConroller {
  ...
  async create(req, res, next) {
    try {
      ...
      const user = await Users.create({ login, password: hashPassword, f_name: fName, s_name: sName, l_name: lName, roleId });
  ...
  async check(req, res) {
    const token = generateJwt(req.user.login, req.user.id, req.user.basketId)
    return res.json(token)
  ...
  async login(req, res, next) {
    ...
    const token = generateJwt(user.login, user.id, basket.id);
  ...
  async getAll(req, res, next) {
    ...
    const users = await Users.findAll({ include: [{ model: Basket }, { model: Roles }] });
  ...
  async getOne(req, res, next) {
    ...
    const orders = await Orders.findOne({where:{userId:user.id}});
    const basket = await Basket.findOne({ where: { userId: user.id }, attributes: ['userId', 'id'], include: { model: BasketItems } })
    return res.json({ user, basket,orders });
